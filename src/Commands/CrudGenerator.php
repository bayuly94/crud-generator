...
namespace Bayuly94\CrudGenerator\Commands;
...
class CrudGenerator extends GeneratorCommand
{
    ...
    protected function buildController(): static
    {
        ...
        foreach ($this->getFilteredColumns() as $column) {
            if ($idx_key == 0) {
                $cols .= '$query->where("'.$column.'","like","%".$search."%")';
            } else {
                $cols .= '
        ...
        $replace['{{search_input}}'] = '$search = $request->search;';

        $search_where = "";
        if (strlen($cols) > 0) {
            $search_where = 'where(function($query) use ($search) {
                '.$cols.'
                ;
            })
            ->';
        }
        $replace['{{search_where}}'] = $search_where;


        $sort_by = 'when(request()->sort_col, function($query){
                $query->orderBy(request()->sort_col, request()->sort == "desc" ? "desc" : "asc");
            })
            ->';
